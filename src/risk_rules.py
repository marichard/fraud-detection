...
def score_transaction(tx: Dict) -> int:
    ...
    # IP country differs from account's registered country — strong CNP fraud signal.
    ip_country = tx.get("ip_country")
    account_country = tx.get("country")
    if ip_country and account_country and ip_country != account_country:
        score += 20
